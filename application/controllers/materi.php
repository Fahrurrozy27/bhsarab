...
class Materi extends CI_Controller
{
    ...
    {
        $this->load->model('m_materi');
        $data['materi'] = $this->m_materi->buku1()->result();
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();
        $this->load->view('materi/buku1', $data);
    ...
    {
        $this->load->model('m_materi');
        $data['materi'] = $this->m_materi->buku2()->result();
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();
        $this->load->view('materi/buku2', $data);
        $this->load->view('template/footer');
    }
    ...
    {
        $this->load->model('m_materi');
        $data['materi'] = $this->m_materi->buku3()->result();
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();
        $this->load->view('materi/buku3', $data);
        $this->load->view('template/footer');
    }
